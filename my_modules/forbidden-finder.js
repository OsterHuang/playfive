...
ForbiddenFinder.prototype.setBoardSize = function (pBoardSize) {
...
*/
ForbiddenFinder.prototype.findAllForbiddenPoints = function (moves, boardSize) {
...
ForbiddenFinder.prototype.isGameFinished = function (moves, boardSize, isRenju, isOverlineWin) {
    this.setBoardSize(boardSize);
...
//    console.log(' Isblack %s, IsRenju ', isBlack, isRenju);
    // 
    if (isBlack && isRenju) {
        if (this.isOverline(lastMove.ordinate.x, lastMove.ordinate.y)) {
            return {isFinished:true, winner:'white', reason:'Black is overline.'};
        }
        
        if (this.isDoubleFour(lastMove.ordinate.x, lastMove.ordinate.y)) {
            return {isFinished:true, winner:'white', reason:'Black is double four.'};
        }
        
        if (this.isDoubleThree(lastMove.ordinate.x, lastMove.ordinate.y)) {
            return {isFinished:true, winner:'white', reason:'Black is double three.'};
        }
    }
    
    if (this.isFive(lastMove.ordinate.x, lastMove.ordinate.y, isBlack ? 0 : 1)) {
        return {isFinished:true, winner:(isBlack ? 'black' : 'white'), reason:'Attains five.'};
    }
    
    return {isFinished:false};
}

ForbiddenFinder.prototype.isItDoubleFour = function (pMoves) {
    this.clear();
    ...
    return this.isDoubleFour(lastMove.ordinate.x, lastMove.ordinate.y);
}

exports.ForbiddenFinder = ForbiddenFinder;
